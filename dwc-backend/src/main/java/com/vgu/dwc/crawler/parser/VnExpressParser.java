...
public class VnExpressParser extends BaseParser {
    ...
    public ArticleData parse(String url, String workerId) throws IOException {
        ...
        Element tagElement = doc.selectFirst("ul.breadcrumb li a");

        // Hoặc một cách khác phổ biến của VnExpress là lấy theo class header-content
        if(tagElement == null){
             tagElement = doc.selectFirst("h2.title-detail span a"); 
        }
