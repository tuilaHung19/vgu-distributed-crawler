...
public class RedisQueueManager {
    ...
    public java.util.Map<String, Integer> getFailedCountsByDomain() {
        java.util.Map<String, Integer> counts = new java.util.HashMap<>();
        counts.put("tuoitre.vn", 0);
        counts.put("thanhnien.vn", 0);
        counts.put("vnexpress.net", 0);

        // Lấy tất cả link trong thùng rác ra
        java.util.List<String> failedLinks = jedis.lrange(FAILED_LIST, 0, -1);
        for (String link : failedLinks) {
            if (link.contains("tuoitre.vn")) counts.put("tuoitre.vn", counts.get("tuoitre.vn") + 1);
            else if (link.contains("thanhnien.vn")) counts.put("thanhnien.vn", counts.get("thanhnien.vn") + 1);
            else if (link.contains("vnexpress.net")) counts.put("vnexpress.net", counts.get("vnexpress.net") + 1);
        }
        return counts;
    }
    
 // THÊM HÀM NÀY ĐỂ XÓA SẠCH REDIS
    public void purgeAll() {
        // Xóa toàn bộ key chứa danh sách link trong Redis
        jedis.del(WAITING_LIST);
        jedis.del(FAILED_LIST);
        jedis.del("crawler:processing_list");
        jedis.del("crawler:url_bloom_filter");
        
        System.out.println("🗑️ Đã dọn dẹp sạch sẽ toàn bộ Queue trong Redis!");
    }
}